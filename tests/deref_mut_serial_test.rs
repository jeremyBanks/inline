...
fn test_deref_mut_triggers_write_on_drop() {
    ...
    let mut counter = code_cell::code_cell(0u32);
}
"#;
...

    {
        ...
        let mut counter = code_cell::CodeCell::__new(0u32, path.to_str().unwrap(), line, col);
    ...
    assert!(
        content.contains("code_cell(1u32)"),
...
fn test_deref_mut_no_write_if_unchanged() {
    ...
    let source = r#"fn test() {
    let mut x = code_cell::code_cell(42u32);
}
"#;
    fs::write(&path, source).unwrap();
    ...
    {
        let x = code_cell::CodeCell::__new(42u32, path.to_str().unwrap(), line, col);
    ...
    let content = fs::read_to_string(&path).unwrap();
    assert!(
        content.contains("code_cell(42u32)"),
        "File should still contain original value"
    );
...
fn test_deref_mut_with_complex_mutation() {
    ...
    let source = r#"fn test() {
    let mut s = code_cell::code_cell("hello".to_string());
}
"#;
    fs::write(&path, source).unwrap();
    ...
    {
        let mut s = code_cell::CodeCell::__new(
            "hello".to_string(),
            path.to_str().unwrap(),
            line,
            col,
...
fn test_deref_mut_with_vec() {
    ...
    let source = r#"fn test() {
    let mut v = code_cell::code_cell(vec![1u32, 2u32]);
}
"#;
    fs::write(&path, source).unwrap();
    ...
    {
        let mut v = code_cell::CodeCell::__new(
            vec![1u32, 2u32],
            path.to_str().unwrap(),
            line,
            col,
...
"#;
    ...
    let content = fs::read_to_string(&path).unwrap();
    assert!(
        content.contains("code_cell(9u32)"),
