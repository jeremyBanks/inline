...
fn test_multi_threaded_access() {
    // Create a simple test file with multiple code_cell() calls
...
"#;
    ...
    let index_0 =
        code_cell::runtime::get_macro_index(&test_file, positions[0].0, positions[0].1).unwrap();
    let index_1 =
        code_cell::runtime::get_macro_index(&test_file, positions[1].0, positions[1].1).unwrap();
    let index_2 =
        code_cell::runtime::get_macro_index(&test_file, positions[2].0, positions[2].1).unwrap();
    ...
    let handles: Vec<_> = (0..4)
        .map(|thread_id| {
            ...
            thread::spawn(move || {
                ...
                for i in 0..3 {
                    let tokens = code_cell::runtime::get_macro_tokens_by_index(&test_file, i).unwrap();
    ...
    let modifier = thread::spawn(move || {
        ...
        code_cell::runtime::update_macro_by_index(&test_file_clone, 0, new_tokens).unwrap();
        code_cell::runtime::write_to_disk(&test_file_clone).unwrap();
    });

    modifier.join().unwrap();

    // Verify from main thread that modification is visible
    let tokens = code_cell::runtime::get_macro_tokens_by_index(&test_file, 0).unwrap();
    assert_eq!(tokens.to_string(), "999");

    // Spawn another thread to verify it sees the update
    let test_file_clone = Arc::clone(&test_file);
    let reader = thread::spawn(move || {
        let tokens = code_cell::runtime::get_macro_tokens_by_index(&test_file_clone, 0).unwrap();
        assert_eq!(tokens.to_string(), "999");
    });
...
"#;
    ...
    let handles: Vec<_> = (0..5)
        .map(|i| {
            ...
            thread::spawn(move || {
                ...
                code_cell::runtime::update_macro_by_index(&test_file, 0, new_tokens).unwrap();
                code_cell::runtime::write_to_disk(&test_file).unwrap();
            })
        })
        .collect();
    ...
    let final_tokens = code_cell::runtime::get_macro_tokens_by_index(&test_file, 0).unwrap();
