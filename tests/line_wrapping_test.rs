...
"#;
    ...
    let mut litter_a =
        litter::Litter::__new(vec![1u32, 2u32], path.to_str().unwrap(), a_line, a_col);
    ...
    if positions_after[1].0 != b_line {
        ...
        let mut litter_b = litter::Litter::__new(100u32, path.to_str().unwrap(), b_line, b_col);

        litter_b.set(999u32);
        let final_content = fs::read_to_string(&path).unwrap();
        assert!(
            final_content.contains("litter!(999u32)"),
