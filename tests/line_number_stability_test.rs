...
fn test_line_number_stability_with_multiple_litters() {
    // This tests the REAL concern: does updating the first litter
    // affect the line number of the second litter?
...
"#;
    ...
    for (i, (line, col)) in positions.iter().enumerate() {
        println!("Litter {}: line {}, column {}", i, line, col);
    }
    ...
    // Create litter instances
    let mut litter_a = litter::Litter::__new(1u32, path.to_str().unwrap(), a_line, a_col);
    let mut litter_b = litter::Litter::__new(2u32, path.to_str().unwrap(), b_line, b_col);
    let mut litter_c = litter::Litter::__new(3u32, path.to_str().unwrap(), c_line, c_col);
