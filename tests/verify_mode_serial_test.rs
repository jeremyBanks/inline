...
"#;
    ...
    let mut value = inline::InlineCell::__new(42u32, path.to_str().unwrap(), line, column);
...
#[test]
#[should_panic(expected = "InlineCell verification failed")]
...
fn test_verify_mode_complex_value() {
    ...
    let content = r#"fn test() {
    let x = inline::cell(vec![1u32, 2u32, 3u32]);
}
"#;
    fs::write(&path, content).unwrap();
    ...
    let mut value =
        inline::InlineCell::__new(vec![1u32, 2u32, 3u32], path.to_str().unwrap(), line, column);
